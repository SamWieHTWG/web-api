...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  ret_value = cnc_register_log_message_function(cnc_demo_log_message);
  if (ret_value != ERR_CNC_NOERROR)
    exit(ret_value);
  ret_value = cnc_register_log_message_json_function(cnc_demo_log_message_json);
  if (ret_value != ERR_CNC_NOERROR)
    exit(ret_value);

  /* Activate logging of startup errors. */
  cnc_set_startup_error_log(true);
  cnc_startup_error_log_file("ErrorsWhileStartUp.log");
  ...
  */
  ...
  /* Create menu thread */
  unsigned int menu_activition_bitmap =  MENU_GENERATE_DIAG_DATA | MENU_RESET_KERNEL |
