...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  cnc_set_tcp_object_access(false);

  /* Create menu thread */
  unsigned int menu_activition_bitmap = MENU_RESET_KERNEL | MENU_SHOW_POS |
                                  MENU_START_PROGRAM | MENU_MANUAL_INPUT | MENU_SHUTDOWN | MENU_SHOW_CNC_OBJECTS | MENU_WRITE_CSV;
  cnc_demo_create_menu_thread(menu_activition_bitmap);

  /* Start CNC Web API server thread */
  if (cnc_web_api_start() != 0) {
    printf("Failed to start CNC Web API\n");
  } else {
    printf("CNC Web API started successfully on port 8080\n");
  }
